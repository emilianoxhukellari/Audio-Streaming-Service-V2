...
namespace AudioEngine.Services
{
    ...
    public class AudioEngineService : IAudioEngineService
    {
        ...
        public AudioEngineService(IServiceProvider serviceProvider,
            IAudioEngineConfigurationService audioEngineConfigurationService)
        {
            _serviceProvider = serviceProvider;
            _audioEngineConfigurationService = audioEngineConfigurationService;
            _controller = new Controller(_serviceProvider, _audioEngineConfigurationService);
        ...
        private void LoadUpdateLoop()
        {
            ...
            while (true)
            {
                if (connectedCount != _controller.ConnectedCount 
                    || clientCountLimit 
                    != GetDesktopClientCountLimit())
                {
        ...
        public void SetDesktopAppSongSearchLimit(int limit)
        {
            ...
            {
                var dataAccessConfigurationService = scope
                    .ServiceProvider
                    .GetRequiredService<IDataAccessConfigurationService>();
