    ...
    public class SongDetailsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPost()
        {
            if (SongFile != null)
            {
                ...
                if (success)
                {
                    this.SetTempData(WebApplicationExtensions.Status.Success, 
                        "Song updated successfully.");
                }
                else
                {
                    this.SetTempData(WebApplicationExtensions.Status.Fail, 
                        "Could not update song. Files with different durations are not accepted.");
                }
            }
            else
            {
            ...
            Song = await _songManagerService.GetSongFromDatabaseAsync(Id) ?? new Song();
            return Page();
        }
    }
}
