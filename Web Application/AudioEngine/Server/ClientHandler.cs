...
namespace Server_Application.Server
{
    ...
    {
        private readonly PlaylistSynchronizer _playlistSynchronizerInternalService;
        ...
        public ClientHandler(string clientId, SslStream streamingSSL, SslStream communicationSSL, IServiceScope serviceScope)
        {
            _playlistSynchronizerInternalService = new PlaylistSynchronizer(serviceScope);
        ...
        private void ExecuteCommunicationRequest(string request)
        {
            ...
            if (_isLoggedIn)
            {
                switch (translate[0])
                {
                    case "TERMINATE_SONG_DATA_RECEIVE":
                    ...
                    case "SYNC_DELETE_PLAYLIST":
                        SynchronizeDeletePlaylist();
                        break;

                    case "SYNC_ADD_PLAYLIST":
                        SynchronizeAddPlaylist();
                        break;

                    case "SYNC_RENAME_PLAYLIST":
                        SynchronizeRenamePlaylist();
                        break;

                    case "SYNC_DELETE_SONG":
        ...
        private async Task Register()
        {
            ...
            IAccountService registrationService = _serviceScope.ServiceProvider.GetRequiredService<IAccountService>();
        ...
        private async Task LogIn()
        {
            ...
            UserManager<IdentityUser> userManager = _serviceScope.ServiceProvider.GetRequiredService<UserManager<IdentityUser>>();
        ...
        private void UserLoggedIn(bool state)
        {
            if (state)
            {
                _isLoggedIn = true;
        ...
        private void SearchForSong(string searchString)
        {
            if (searchString != "")
            {
                ...
                ISongManagerService songManagerService = _serviceScope.ServiceProvider.GetRequiredService<ISongManagerService>();
                IAudioEngineConfigurationService audioEngineConfigurationService = _serviceScope.ServiceProvider.GetRequiredService<IAudioEngineConfigurationService>();
