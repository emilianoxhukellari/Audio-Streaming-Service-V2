...
namespace Client_Application.Client.Core
{
    public sealed class MediaPlayer
    {
        ...
        public void PlayPlaylistNext(List<Song> songs)
        {
            for (int i = 0; i < songs.Count; i++)
            {
                if(_queue.Any())
                {
                    _queue.InsertToQueue(_currentSongIndex + 1 + i, songs[i]);

                }
                else
                {
                    _queue.InsertToQueue(_currentSongIndex + i, songs[i]);
                }
            }
            DisplayQueue();
        }
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < packetCount; i++) // Receive all packets unless terminated. Termination comes as command from server
                    {
                        ...
                        if (currentType.SequenceEqual(DATA))
                        {
                        ...
                        else if (currentType.SequenceEqual(EXIT)) // Terminate
                        {
                            receivedAll = false;
                            break;
                        }
                ...
                {
                    if (_waveOutPlayer.IsPlayerActive() && mode.SequenceEqual(MOD2) && receivedAll)
                    {
